...
class MultiPostViewModel
    ...
    constructor(
        ...
        fun onBookConditionSelected(condition: BookCondition) = updateSheet { it.copy(condition = condition, conditionError = null) }
        ...
        fun onBookTypeOverrideSelected(type: ListingType?) = updateSheet { it.copy(typeOverride = type) }

        fun onBookPhotosSelected(uris: List<Uri>) =
            updateSheet { sheet ->
                val combined = (sheet.photoUris + uris).take(5)
                sheet.copy(photoUris = combined)
            }

        fun onBookPhotoRemoved(uri: Uri) = updateSheet { it.copy(photoUris = it.photoUris - uri) }

        fun onBookCoverPhotoSet(uri: Uri) =
            updateSheet { sheet ->
...
private fun CachedStagedBook.toDomain(): StagedBook =
    StagedBook(
        id = id,
        sessionId = sessionId,
        bundleId = bundleId,
...
private fun CachedStagedBundle.toDomain(): StagedBundle =
    StagedBundle(
        id = id,
        sessionId = sessionId,
        name = name,
